document.addEventListener("DOMContentLoaded", async () => {
  ...
  const els = {
    ...
    editor: $("modalNewRoutine"), name: $("newRoutineName"), weekdays: $("newRoutineWeekdays"), exercises: $("newExercisesList"), muscleFilter: $("muscleGroupFilter"), suggestions: $("exerciseSuggestions"), suggestionHelp: $("exerciseSuggestionHelp"),
    ...
    repeatStrengthLabel: $("dayAssignRepeatStrengthLabel"), repeatCardioLabel: $("dayAssignRepeatCardioLabel"), cardioDistance: $("cardioDistanceInput"), cardioWeight: $("cardioWeightInput")
  };
  ...
  function renderCalendar() {
    ...
    calendarManager.getCurrentWeekDays().forEach(day => {
      ...
      else if (day.scheduled?.type === "cardio") status = `<span class='day-status-badge status-cardio-scheduled'>${day.scheduled.recurring ? "↻ " : "🏃 "}${esc(day.scheduled.activity)}</span>`;
      else if (day.scheduled) status = `<span class='day-status-badge status-scheduled'>${day.scheduled.recurring ? "↻ " : ""}${esc(routines.find(r => r.id === day.scheduled.routineId)?.name || "Rutina eliminada")}</span>`;
  ...
  function openDay(day) {
    activeDay = day; const weekdayName = ["lunes", "martes", "miércoles", "jueves", "viernes", "sábado", "domingo"][day.dayIndex]; els.assignTitle.textContent = `Planificar ${day.dayName} ${day.dayNumber}`; els.assignSub.textContent = day.isCompleted ? "Esta sesión ya fue completada." : day.scheduled?.recurring ? `Esta actividad se repite todos los ${weekdayName}.` : "Elige una actividad puntual o repítela cada semana.";
    ...
    els.repeatStrength.checked = day.scheduled?.type === "strength" && Boolean(day.scheduled.recurring); els.repeatCardio.checked = day.scheduled?.type === "cardio" && Boolean(day.scheduled.recurring);
    els.repeatStrengthLabel.textContent = `Repetir todos los ${weekdayName}`; els.repeatCardioLabel.textContent = `Repetir todos los ${weekdayName}`;
    ...
    els.repeatStrength.onchange = () => { $("btnAssignRoutineToDay").textContent = `✓ Guardar ${els.repeatStrength.checked ? `todos los ${weekdayName}` : "solo este día"}`; };
    els.repeatCardio.onchange = () => { $("btnAssignCardioToDay").textContent = `🏃 Guardar ${els.repeatCardio.checked ? `todos los ${weekdayName}` : "solo este día"}`; };
  ...
  function initCardio() {
    // Compatibilidad segura si el navegador conserva una versión anterior del módulo de cardio en caché.
    ...
    const metrics = (syncInputs = false) => { if (syncInputs) { els.cardioDistance.value = cardioManager.distanceKm.toFixed(2); els.cardioWeight.value = cardioManager.weightKg.toFixed(1); } $("cardioPaceVal").textContent = cardioManager.calculatePace(cardioManager.distanceKm); $("cardioCaloriesVal").textContent = cardioManager.estimateCalories(cardioManager.selectedActivity); };
    const updateDistance = () => { const value = Number(els.cardioDistance.value); if (Number.isFinite(value) && value >= 0.01 && value <= 999.99) { cardioManager.distanceKm = value; metrics(); } };
    const updateWeight = () => { const value = Number(els.cardioWeight.value); if (Number.isFinite(value) && value >= 30 && value <= 250) { cardioManager.weightKg = value; metrics(); } };
    els.cardioDistance.addEventListener("input", updateDistance); els.cardioDistance.addEventListener("change", () => { updateDistance(); metrics(true); }); els.cardioWeight.addEventListener("input", updateWeight); els.cardioWeight.addEventListener("change", () => { updateWeight(); metrics(true); });
    $("btnCardioStart").onclick = () => { updateDistance(); updateWeight(); if (!(cardioManager.distanceKm > 0)) return alert("Indica una distancia válida antes de empezar."); $("btnCardioStart").classList.add("hidden"); $("btnCardioPause").classList.remove("hidden"); cardioManager.startStopwatch(time => { els.cardioDigits.textContent = time; metrics(); }); }; $("btnCardioPause").onclick = () => { cardioManager.pauseStopwatch(); $("btnCardioPause").classList.add("hidden"); $("btnCardioStart").classList.remove("hidden"); }; $("btnCardioReset").onclick = () => { cardioManager.resetStopwatch(); els.cardioDigits.textContent = "00:00"; metrics(true); }; $("btnDecDist").onclick = () => { cardioManager.distanceKm = Math.max(.01, Number((cardioManager.distanceKm - .25).toFixed(2))); metrics(true); }; $("btnIncDist").onclick = () => { cardioManager.distanceKm = Number((cardioManager.distanceKm + .25).toFixed(2)); metrics(true); };
    ...
    $("btnSaveCardioSession").onclick = async () => { if (!cardioManager.getTotalElapsedSeconds()) return alert("Inicia el cronómetro antes de guardar."); updateDistance(); updateWeight(); const session = await cardioManager.saveCardioSession({ distanceKm: cardioManager.distanceKm, weightKg: cardioManager.weightKg, rpe: cardioManager.rpeValue }); const minutes = Math.max(1, Math.round(session.durationSeconds / 60)); const reward = gamificationEngine.recordWorkoutCompletion("cardio", { distanceKm: session.distanceKm, minutes }); calendarManager.markCompleted(plannedDay || calendarManager.getCurrentWeekDays().find(day => day.isToday).isoDate, { type: "cardio", distanceKm: session.distanceKm, minutes }); plannedDay = null; $("summaryVolume").textContent = `${session.distanceKm} km`; $("summarySets").textContent = session.formattedTime; $("summaryXP").textContent = `+${reward.xpGained} XP`; $("summarySessionsText").textContent = reward.totalSessions; els.summary.classList.remove("hidden"); renderProgress(); renderCalendar(); metrics(true); };
    metrics(true);
  }
  ...
  $("btnNewRoutineModal").onclick = () => openEditor(); $("btnEditCurrentRoutine").onclick = () => activeRoutineId ? openEditor(activeRoutineId) : openEditor(); $("btnCloseModal").onclick = $("btnCancelRoutine").onclick = () => els.editor.classList.add("hidden"); els.muscleFilter.onchange = renderExerciseSuggestions; $("btnAddExerciseRow").onclick = () => { draft.push({ id: uid("exercise"), name: "", sets: 3, defaultReps: 10, defaultWeight: 0, perSide: false }); renderDraft(); }; $("btnSaveRoutine").onclick = saveRoutine;
  ...
  $("btnAssignRoutineToDay").onclick = () => { if (!activeDay || !els.assignSelect.value) return alert("Selecciona una rutina guardada."); if (els.repeatStrength.checked) calendarManager.assignStrengthWeekly(activeDay.dayIndex, els.assignSelect.value, activeDay.isoDate); else calendarManager.assignStrengthToDay(activeDay.isoDate, els.assignSelect.value); els.assign.classList.add("hidden"); renderCalendar(); };
  $("btnAssignCardioToDay").onclick = () => { if (!activeDay) return; if (els.repeatCardio.checked) calendarManager.assignCardioWeekly(activeDay.dayIndex, "Carrera", activeDay.isoDate); else calendarManager.assignCardioToDay(activeDay.isoDate); els.assign.classList.add("hidden"); renderCalendar(); }; $("btnClearDayAssign").onclick = () => { if (activeDay) calendarManager.clearDay(activeDay.isoDate); els.assign.classList.add("hidden"); renderCalendar(); }; $("btnCloseDayAssign").onclick = () => els.assign.classList.add("hidden");
